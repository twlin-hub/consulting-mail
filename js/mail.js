...
function generateMailBody(data) {
    ...
    // 1️⃣ 방향 제안
    mail += generateSection('1️⃣ 방향 제안', getDirectionContent(name, monetization, status));
...
function getDirectionContent(name, monetization, status) {
    if (monetization && monetization.includes('대행사 창업')) {
        ...
        if (status && status.includes('운영')) {
            return MAIL_TEMPLATES.direction['병행_운영중'](name);
        } else if (status && (status.includes('원고작가') || status.includes('프리랜서') || status.includes('활동'))) {
            return MAIL_TEMPLATES.direction['병행_활동중'](name);
        } else {
